...
#                print "e_dos", e_dos
                bin_min=min(range(len(x_axis)), key=lambda i: abs(x_axis[i]-(e_dos-0.1)))
                bin_max=min(range(len(x_axis)), key=lambda i: abs(x_axis[i]-(e_dos+0.1)))
                for bin in range(bin_min,bin_max):
                    data_dos_all[bin] += gaussian(x_axis[bin],e_dos,broadening)*kpoints_weights[k,3]
                if e1< fermi_level:
                    i_homo = i
            i_lumo=i_homo+1
            orb_min=min(range(len(eigenvalues[k,s,:i_lumo])), key=lambda j: abs(eigenvalues[k,s,j]-(fermi_level-0.5)))
            orb_max=min(range(len(eigenvalues[k,s,i_lumo:])), key=lambda j: abs(eigenvalues[k,s,j]-(fermi_level+0.5)))
            for i in range(orb_min,i_lumo):
                for f in range(i_lumo,orb_max):
                        e = eigenvalues[k,s,f]-eigenvalues[k,s,i]
#                        if e <= 0.5:
#                        if (e2-e1) <= 0.5:
                        print "e", e
                        bin_min=min(range(len(x_axis)), key=lambda i: abs(x_axis[i]-(e-0.05)))
                        bin_max=min(range(len(x_axis)), key=lambda i: abs(x_axis[i]-(e+0.05)))
                        for bin in range(bin_min,bin_max):
