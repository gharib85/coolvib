...
def dos_binning(eigenvalues,broadening=0.005, bin_width=0.002,
        coeffs=None):
    """
                ...
                if eigenvalues[k,s,i] > fermi_level:
                    ...
                    for f in range(len(eigenvalues[k,s,:])):
                        if eigenvalues[k,s,f] < fermi_level:
                            ...
                            #hit_the_window+=1     
    data=np.asarray(sorted(i for i in energy if i <=1))
...
#    print path_1
    np.savetxt(path_1+'/test_scan_kpoints_{0}.txt'.format(index), np.vstack([x,y]).T,fmt=['%0.8f','%0.8f']) 
