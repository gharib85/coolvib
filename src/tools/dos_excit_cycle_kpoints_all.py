...
#    psi = siesta_read_coefficients(path+"/eq/MyM")
    ...
    for k in range(len(kpoints_weights)):
        for k1 in range(len(kpoints_weights)):
            for s in range(n_spin):
                eigs=eigenvalues[k,s,orb_min:orb_max]-fermi_level
                eigs1=eigenvalues[k1,s,orb_min:orb_max]-fermi_level
                i_homo = 0
                i_homo1 = 0
                i_lumo = 0
                i_lumo1 = 0
                ...
                for i,ei in enumerate(eigs[:i_lumo]):
                    for f,ef in enumerate(eigs1[i_lumo1:]):
                            e = ef-ei
                            for bin in range(num_bins):
                                data_excit_all[bin] += gaussian(x_axis[bin],e,broadening/2)*kpoints_weights[k,3]*kpoints_weights[k1,3]
    np.savetxt(path_1+'/excit_scan_kpoints_all_{0}.txt'.format(index), np.vstack([x_axis,data_excit_all]).T,fmt=['%0.8f','%0.8f']) 
    np.savetxt(path_1+'/dos_scan_kpoints_all_{0}.txt'.format(index), np.vstack([x_axis,data_dos_all]).T,fmt=['%0.8f','%0.8f']) 
