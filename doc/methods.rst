...
where the sums run over spin, crystal momentum vectors :math:`\mathbf{k}` and all pairs of eigenvectors :math:`\nu` and :math:`\nu'` with the state occupations :math:`f` given by the Fermi-Dirac distribution and the coupling elements :math:`g^{\mathbf{q}j}_{s\mathbf{k+q}\nu',s\mathbf{k}\nu}`
...
The relaxation rate relates to the lifetime of the mode by :math:`\tau=1/\Gamma` and to the vibrational linewidth by :math:`\gamma=\Gamma\hbar`. In eq. :eq:`coupling` :math:`\mathbf{e}_{\mathbf{q}j}` is the atomic displacement vector corresponding to a given vibrational motion :math:`\omega_{\mathbf{q}j}` described by a wave-vector :math:`\mathbf{q}` and a quantum number :math:`j`. :math:`\nabla_{\mathbf{R}}` represents the derivative vector with respect to all cartesian atomic positions.

It is often assumed in literature [Head-Gordon1995]_, [Hellsing1984]_ that perturbations :math:`\hbar\omega` due to vibrations are small and the density of state is almost constant in a large energy range around the Fermi level. This so-called quasi-static approximation leads to following simplification:
